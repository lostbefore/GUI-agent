...
Follow the active plan step before later pending steps. Return finish only after the screenshot proves the goal is complete.
Allowed actions: click, double_click, context_open, type, press, hotkey, scroll, drag, wait, finish.
Schema: {"action":"click","step_id":1,"x":0,"y":0,"button":"left","text":"","key":"","keys":[],"start":[0,0],"end":[0,0],"amount":0,"duration":0,"reason":"..."}.
...
class DesktopAgent:
    ...
    @staticmethod
    def _normalize_action(action: str, parameters: dict[str, Any]) -> tuple[str, dict[str, Any]]:
